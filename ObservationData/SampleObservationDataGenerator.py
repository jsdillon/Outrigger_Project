...
def SampleObservationDataGenerator(configFile = "outrigger_config.txt"):
    print "Now generating sample obeservation informaiton..."    
    ...
    config.read(scriptDirectory + '/../' + configFile)
    ...
    frequencyRange = map(float, config.get('Pipeline Settings','frequencyRange').split())
    frequencyList = frequencyList[(frequencyList >= frequencyRange[0]) * (frequencyList <= frequencyRange[1])]
...
if __name__ == "__main__":
    SampleObservationDataGenerator()